...
#include <laser_data_fusion/simLaserInfo.h>
#include <vector>
#include <algorithm>
#include <cmath>
...
void PointAnalysis(double range, double angle, const double offset[]) {
  // Change coordinate and angle from laser to base_link
  double x = range * std::cos(angle) + offset[0];
  double y = range * std::sin(angle) + offset[1];
...
void ScanFilter(const sensor_msgs::LaserScan& scan, int laserIndex) {
  for (int i = 0; i < scan.ranges.size(); ++i) {
    ...
    PointAnalysis(scan.ranges[i], angle, LASER_OFFSET[laserIndex]);
  }
}
...
void MergeScanMessage(sensor_msgs::LaserScan& scan, ros::Time scan_time) {
  ...
  scan.header.frame_id = "laser";//important!!!!!!!!!!!!!!!!!
  scan.angle_min = 0;
  scan.angle_max = 2 * PI;
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "sim_laser_fusion");
  ros::NodeHandle n;

  std::for_each(updateTime.begin(), updateTime.end(), [](ros::Time timeNow) { timeNow = ros::Time::now(); });
  ros::Subscriber sub1 = n.subscribe("/sim_scan0", 1, Laser0ToBase);
  ros::Subscriber sub2 = n.subscribe("/sim_scan1", 1, Laser1ToBase);
